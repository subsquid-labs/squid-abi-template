...
(0, util_internal_1.runProgram)(async function () {
    ...
    let out = new util_internal_code_printer_1.FileOutput(`./src/processor.ts`);
    out.line(`import {Store, TypeormDatabase} from '@subsquid/typeorm-store'`);
    out.line(`import {EvmBatchProcessor, EvmBlock, BatchProcessorItem, BatchProcessorLogItem, BatchHandlerContext, BatchProcessorTransactionItem} from '@subsquid/evm-processor'`);
    ...
    out.indentation(() => {
        if (opts.event)
            out.line(`let events: EvmEvent[] = []`);
        if (opts.transaction)
            out.line(`let transactions: EvmTransaction[] = []`);
        ...
        out.line(`}`);
        if (opts.event)
            out.line(`await ctx.store.save(events)`);
        if (opts.transaction)
    ...
    out.line();
    if (opts.transaction) {
        out.line(`function parseTransaction(ctx: Context, block: EvmBlock, item: BatchProcessorTransactionItem<typeof processor>) {`);
        out.indentation(() => {
